...
There are 19 BIOs commonly used in species distribution models (SDMs):
  ...
  - **BIO03** (adimensional) -- annual mean isothermality = $\frac{1/12 \sum_{i=1}^{12} (\text{tmax}_i - \text{tmin}_i)}{\text{max(tmax)} - \text{min(tmin)}}$. Usually, this variable is returned multiplied by 100, but I preferred not to do so; this is a adimensional variables and the only reason to multiply by 100 is to save disk space by saving it as a number type that takes less space. Note that CHELSA documentation about this variable (https://chelsa-climate.org/wp-admin/download-page/CHELSA_tech_specification.pdf, p. 14) is wrong.
...
plot(b10[["start.quarter"]], type = "classes", main = "First month of quarter")
```
